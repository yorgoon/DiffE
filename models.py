...
class Decoder(nn.Module):
    ...
    def forward(self, x0, encoder_out, diffusion_out):
        ...
        # up22, up33, up44 = up
        # up2 = self.up2(dn33)
        up2 = self.up1(torch.cat([dn3, dn33.detach()], 1)) # 256 -> 512
        up3 = self.up2(torch.cat([up2, dn22.detach()], 1)) # 512 -> 1024
        out = self.up3(torch.cat([self.pool(x0), self.pool(x_hat.detach()), up3, dn11.detach()], 1)) # 1024 -> 2048
