...
def minmax_norm(data):
    ...
    norm_data = (data - min_vals.unsqueeze(-1)) / (max_vals.unsqueeze(-1) - min_vals.unsqueeze(-1))
...
def load_data(root_dir, subject, session):
    ...
    file = mat73.loadmat(root_dir + filename + '.mat') # Dictionary with keys Xi_train, Yi_train, Xi_test, Yi_test
...
def get_dataloader(X, Y, batch_size, batch_size2, seed, shuffle=True):
    X_train, X_test, Y_train, Y_test  = train_test_split(X, Y, test_size=0.2 ,shuffle=shuffle, stratify=Y, random_state=seed)
